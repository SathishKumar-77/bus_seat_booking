...
const BusList = () => {
  ...
  useEffect(() => {
    if (from && to) {
      ...
      .then(res => {
        ...
        const fullDayName = selectedDate.toLocaleDateString('en-US', { weekday: 'long' }); // e.g., "Sunday"
        const shortDay = dayMap[fullDayName]; // e.g., "Sun"
      
        const filteredBuses = allBuses.filter(bus =>
          Array.isArray(bus.recurringDays) &&
          bus.recurringDays.includes(shortDay)
        );
  ...
  const formatDuration = (departure, arrival) => {
    ...
    // Create two date objects on same day
    ...
    // If arrival is less than departure, assume it's the next day
    if (arr <= dep) {
...
const handleBookNow = (busId) => {
  ...
  Seater: ₹{bus.fare?.seater?.toFixed(2) || 'NA'} <br />
  Sleeper: ₹{bus.fare?.sleeper?.toFixed(2) || 'NA'}
</h3>
                              ...
                              {bus.operator.rating && (
                                <span className="ms-2">
                                  {Array.from({ length: 5 }).map((_, i) => (
                                    <i 
                                      key={i}
                                      className={`fas fa-star${i < Math.floor(bus.operator.rating) ? '' : '-half-alt'}`}
                                      style={{ color: i < bus.operator.rating ? '#ffc107' : '#e4e5e9' }}
                                    />
                                  ))}
                                </span>
                              )}
